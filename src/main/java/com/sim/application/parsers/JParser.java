...
import com.sim.application.techniques.ThrowableConsumer;
...
public final class JParser
{
    ...
    private static ParserConfiguration.LanguageLevel selectedLanguageLevel = ParserConfiguration.LanguageLevel.JAVA_10;
    private static Charset selectedCharEncoding = StandardCharsets.UTF_8;

    private JParser() { }

    public static void init(String projectDir) {
        JParser.projectDir = Paths.get(projectDir);
        setupConfig(selectedLanguageLevel, selectedCharEncoding);
        projectRoot = collectionStrategy.collect(JParser.projectDir);
    }
    ...
    public static CompilationUnit reparse(CompilationUnit unit) {
        ...
        var parserConfiguration = JParser.getSourceRoot(storage.getSourceRoot()).getParserConfiguration();
