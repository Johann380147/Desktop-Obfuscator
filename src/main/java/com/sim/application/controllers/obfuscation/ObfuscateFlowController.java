...
public final class ObfuscateFlowController extends Technique {
    ...
    public void execute(BiMap<JavaFile, CompilationUnit> sourceFiles, ClassMap classMap, List<Problem> problemList) throws FailedTechniqueException {
        new InsertMethodVariable().updateContents(sourceFiles);
    }

    private static class InsertMethodVariable
    {
        ...
        protected MethodDeclaration generateDummyMethod(boolean isInterface) throws NoSuchAlgorithmException
        {
            ...
            if (isInterface)
            {
                md.addModifier(Modifier.Keyword.DEFAULT);
            } else
            {
                //select random modifier
                switch (rnd.nextInt(3)) {
                    case 0:
                        ...
                        break;
                    case 1:
                        md.addModifier(Modifier.Keyword.PRIVATE);
                        break;
                    case 2:
                        md.addModifier(Modifier.Keyword.PROTECTED);
                        break;
                    default:
                        break;
                }
                ...
                if (rnd.nextBoolean())
                    md.addModifier(Modifier.Keyword.STATIC);
            }
